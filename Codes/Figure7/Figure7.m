...
for iroi = 2:nroi
    ...
    cl = cl(3:5,:);
    for icong = 1:3
        ipa             = coefs_cong(:,:,icong);
        y               = mean(ipa);
        [~,p,ci]        = ttest(ipa);
        e               = (ci(2,:)-ci(1,:))/2;
        lineProps.col   = {cl(icong,:)};
        lineProps.style = '-';
        mseb(1:itr,y,e,lineProps,1);
        for i = 1:itr
            ip = p(i);
            iy = y(i);
            if ip < 0.05
                ...
                if iy>0
                    plot(ix + i,ci(2,i)+dys,'*','linewidth',lw,'color',cl(icong,:))
                else
                    plot(ix + i,ci(1,i)-dys,'*','linewidth',lw,'color',cl(icong,:))
                end
            end
        end
    end
    ...
    set(gca,'ytick',-0.1:0.1:0.1,'fontsize',fz,'xtick',1:6,'XTickLabel',(1:6)*2.2 - 3.3)
    grid on
   
    set(gcf,'PaperUnits','inches','PaperPosition',[0 0 2500 500]/150)
    saveas(gcf,['result_' fmribold.iboldname{iroi} '.png'])
